...
# Test 2: Verify Email
echo ""
echo "2️⃣  Testing POST /auth/verify-email"
if [ ! -z "$VERIFY_TOKEN" ]; then
  ...
  echo "⚠️  Skip - No verify token (configure SMTP in .env)"
fi
...
echo ""
echo "4️⃣  Testing GET /auth/me (Protected)"
if [ ! -z "$ACCESS_TOKEN" ]; then
...
echo ""
echo "5️⃣  Testing POST /auth/refresh-token"
...
echo ""
echo "6️⃣  Testing POST /auth/forgot-password"
...
echo ""
echo "7️⃣  Testing POST /auth/reset-password"
if [ ! -z "$RESET_TOKEN" ]; then
  ...
  echo "⚠️  Skip - No reset token (configure SMTP in .env)"
fi

# Test 8: Logout
echo ""
echo "8️⃣  Testing POST /auth/logout (Protected)"
if [ ! -z "$ACCESS_TOKEN" ]; then
...
echo ""
echo "9️⃣  Testing GET /auth/me with invalid token"
